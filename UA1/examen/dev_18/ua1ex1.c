/*
NOMBRE: Guillermo Jiménez Torres
FECHA: 26/10/2022
PARTE PRÁCTICA
...
int main()
{
	 pid_t pid, pid_hijo1, pid_hijo2; //Creamos el padre y los hijos

	pid = fork(); //Creamos el fork

	if (pid == 0) {

		sleep(3);

	printf("Hijo[1] pid = %d y mi pid = %d\n",
			getpid(), getppid());
	}
		...
		if (pid_hijo1 == 0) {
			sleep(2);
			printf("Hijo[2] --> pid = %d y mi pid = %d\n",
				getpid(), getppid());
		}
		else {
			pid_hijo2 = fork();
			if (pid_hijo2 == 0) {
				...
				printf("Hijo[3] --> pid = %d y mi pid = %d\n",
					getpid(), getppid());
			}
