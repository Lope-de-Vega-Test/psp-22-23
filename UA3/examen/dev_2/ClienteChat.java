...
public class ClienteChat implements Runnable {
	...
	public void run() {
		...
		while (repetir) {
			try {
				texto = fentrada.readUTF(); /*Rellenar check??*/
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			cliente = new ClienteChat(s,nombre);/* Rellenar check */
		...
		while(repetir)
		{
			String texto = bufferedReader.readLine(); /* Rellenar check ??*/
			if(texto.length()>0)
			{
				if(texto.equals("*"))
				{
