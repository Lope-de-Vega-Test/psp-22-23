...
public class HiloServidorChat extends Thread {
	...
	public HiloServidorChat(Socket s, ComunHilos comun) {
		...
		try {
			...
			fentrada = new DataInputStream(socket.getInputStream());/* Rellenar check */
		} catch (IOException e) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				comun.setACTUALES(comun.getACTUALES() - 1);/* Rellenar check */
				System.out.println("NUMERO DE CONEXIONES ACTUALES: " + comun.getACTUALES());
	...
	private void EnviarMensajesaTodos(String texto) {/* Rellenar check */
		int i;
